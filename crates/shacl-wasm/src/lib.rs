...
impl Validator {
    ...
    pub fn from_text(text: &str, format: &str, base: Option<String>) -> Result<Validator, JsValue> {
        ...
        let shapes = Shapes::compile(&shapes_graph, &mut store, &vocab).map_err(err)?;
        Ok(Validator {
            store,
            vocab,
            shapes,
    ...
    pub fn validate_text(
        ...
        let data = parse_graph(text, format, &base, 1, &mut self.store)?;
    ...
    fn build_report(&self, report: ValidationReport) -> Result<Report, JsValue> {
        ...
        let results = report
            ...
            .iter()
            .map(|r| JsResult {
                focus_node: self.term(r.focus_node),
                path: r.path.map(|t| self.term(t)),
                value: r.value.map(|t| self.term(t)),
                severity: self.term(r.severity),
                source_shape: r.source_shape.map(|t| self.term(t)),
                component: self.term(r.source_constraint_component),
                message: r
                    .messages
                    .iter()
                    .map(|&m| self.term(m))
    ...
    /// Renders a term the way an RDF/JS consumer expects `.value` to read: an
    /// IRI bare, a blank node as `_:label`, a literal as its lexical form.
    fn term(&self, id: TermId) -> String {
        if let Some(iri) = self.store.iri(id) {
            return iri.to_string();
        }
        if let Some(lex) = self.store.lexical_form(id) {
            return lex.to_string();
        }
        // Blank nodes and anything else fall back to the engine's own oxrdf
        // rendering, which gives `_:label` for a blank node.
        self.store.to_oxrdf(id).to_string()
    }
}
...
pub fn validate_turtle_once(
    ...
    let mut validator = Validator::from_turtle(shapes, base.clone())?;
